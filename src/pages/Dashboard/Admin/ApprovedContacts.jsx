    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-slate-500 dark:text-slate-400 mt-1">Manage contact information requests</p>
            </div>
            ...
            {requests.length === 0 ? (
                <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-black/30 border border-slate-100 dark:border-slate-700 p-12 text-center">
                    <div className="w-20 h-20 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center mx-auto mb-6">
                        <FaEnvelope className="text-4xl text-emerald-300 dark:text-emerald-400" />
                    </div>
                    <h2 className="text-xl font-bold text-slate-700 dark:text-slate-200 mb-2">No Contact Requests</h2>
                    <p className="text-slate-500 dark:text-slate-400">There are no contact requests to manage.</p>
                </div>
            ) : (
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-700 dark:to-slate-800">
                                    <th className="text-left px-6 py-4 text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">Requester</th>
                                    <th className="text-left px-6 py-4 text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">Email</th>
                                    <th className="text-left px-6 py-4 text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">Biodata ID</th>
                                    <th className="text-left px-6 py-4 text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">Status</th>
                                    <th className="text-left px-6 py-4 text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">Action</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                {requests.map((request, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{request.requesterEmail}</td>
                                        <td className="px-6 py-4">
                                            <span className="inline-flex items-center gap-1 px-3 py-1 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-full">
                                        ...
                                        <td className="px-6 py-4">
                                            {request.status === 'approved' ? (
                                                ...
                                                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-xs font-bold rounded-full">
